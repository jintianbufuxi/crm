...
async function queryDepart() {
    ...
    if (department) {
        department = JSON.parse(department); //类型转换,串转对象
        if (new Date().getTime() - department.time <= 86400000) {
            return department.data;
        }
    }
    result = await axios.get('/department/list'); //调接口数据
    localStorage.setItem('department', JSON.stringify({ //然后把数据放到本地
        time: new Date().getTime(),
        data: result
    }));
    return result; //谁调用了这个借口，谁就可以获取部门信息
}
...
async function queryJob() {
    ...
    if (job) {
        ...
        if (new Date().getTime() - job.time <= 86400000) {
            return job.data; //若缓存中有数据，那么就走数据
        }
    }
    result = await axios.get('/job/list'); //调用接口获取数据
    localStorage.setItem('job', JSON.stringify({
        time: new Date().getTime(),
        data: result
    })); //缓存数据
    return result; //获取职务数据
}